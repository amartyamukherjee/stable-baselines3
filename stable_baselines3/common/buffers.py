...
class HJBRolloutBuffer(RolloutBuffer):
    def __init__(
        self,
        buffer_size: int,
        observation_space: spaces.Space,
    ...
    def reset(self) -> None:
        self.dynamics = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        super().reset()
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                next_non_terminal = 1.0 - dones
                next_values = last_values
                next_state = self.observations[step]
            else:
                next_non_terminal = 1.0 - self.episode_starts[step + 1]
                next_values = self.values[step + 1]
                next_state = self.observations[step+1] * next_non_terminal + self.observations[step] * self.episode_starts[step + 1]
            delta = self.rewards[step] + self.gamma * next_values * next_non_terminal - self.values[step]
            last_gae_lam = delta + self.gamma * self.gae_lambda * next_non_terminal * last_gae_lam
            self.advantages[step] = last_gae_lam
            self.dynamics[step] = (next_state - self.observations[step]) / self.time_step_size
        # TD(lambda) estimator, see Github PR #375 or "Telescoping in TD(lambda)"
        # in David Silver Lecture 4: https://www.youtube.com/watch?v=PnHCvfgC_ZA
        self.returns = self.advantages + self.values
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        data = (
            ...
            self.advantages[batch_inds].flatten(),
            self.returns[batch_inds].flatten(),
            self.dynamics[batch_inds].flatten(),
        )
        return HJBRolloutBufferSamples(*tuple(map(self.to_torch, data)))
